...
- **Public Assessment Grouping System**: Complete campaign/group tracking system for public assessments. URLs can include `?group=xyz` parameter to automatically categorize responses (e.g., `/public/1?group=campanha_marketing`). **IMPORTANT CONCEPTUAL CHANGE (2025-10-19)**: Grouping is now based on the combination of TAG + ASSESSMENT TYPE ID. This means `/public/3?group=teste` and `/public/7?group=teste` create SEPARATE groups (same tag, different assessment types). Admin dashboard includes "Grupos" menu item showing all unique (tag, type) combinations. Each group row displays the tag name and assessment type badge. URLs follow pattern `/admin/grupos/<tag>/<tipo_id>` for precise filtering. Includes database migration `adicionar_coluna_grupo.sql` with indexed grupo column for optimized queries. Perfect for tracking marketing campaigns by assessment type.
- **Interactive Chart Visualizations**: Group statistics page features multiple visualization modes using Chart.js: vertical bars, horizontal bars, radar chart, and traditional table view. Users can switch between visualizations via button selector. Charts apply score-based color coding (green ≥80%, yellow 60-79%, red <60%) consistently across all modes. Radar chart includes colored data points for clear performance visualization. Tooltips correctly display percentages for all chart types with mode-specific parsing (parsed.y for vertical, parsed.x for horizontal, parsed.r for radar). Responsive design maintains readability on all devices.
- **Auto-Refresh Dashboard**: Group statistics page includes customizable auto-refresh functionality with intervals: 5s (fast), 10s, 20s, 1min, or disabled. User preference saved in localStorage. Updates statistics, charts, and table formatting in real-time via AJAX polling to `/admin/grupos/<tag>/<tipo_id>/api` endpoint. Visual indicators show active (blue) or disabled (red) state. Table mode preserves complete formatting including progress bars, colored badges, and Bootstrap classes during refresh.
- **Grupos - Filtros, Exclusão e Grupo Geral**: Enhanced group management with three major features: (1) Filters on /admin/grupos page - dropdown for assessment type filtering, search field for tag filtering with ILIKE, and "Clear Filters" button; (2) Group deletion - delete button for specific groups (not available for general groups) with JavaScript confirmation, removes ALL public assessments in the group including cascaded responses and leads via POST to `/admin/grupos/<tag>/<tipo_id>/delete`; (3) General group for each assessment type - shows statistics for ALL responses of a type (with or without tag), displayed at top of list with 📊 icon, accessible via `/admin/grupos/geral/<tipo_id>`, includes API endpoint for auto-refresh, cannot be deleted. **Important logic**: Responding without `?group=` feeds only the general group; responding with `?group=xyz` feeds both the general group and the specific group xyz. Deployment script: `aplicar_grupos_filtros_exclusao_geral.sh`.
